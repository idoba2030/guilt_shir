...
process_RL <- function(file_path) {
  ...
  df=df%>%filter(trial_name=="reward")%>%
    ...
    rename(trial=trial_num,ch_shape=shape_selected,ch_key=key_selected,shape_left=left_shape,shape_right=right_shape)%>%
    mutate(block=block+1,rt=rt,ch_key=as.integer(ch_key),
           unch_shape=if_else(ch_shape==shape_right,shape_left,shape_right),
           prob_ch=case_when(ch_shape==0~prob1,
                             ch_shape==1~prob2,
                             ch_shape==2~prob3,
                             ch_shape==3~prob4),
           prob_unch=case_when(unch_shape==0~prob1,
                               unch_shape==1~prob2,
                               unch_shape==2~prob3,
                               unch_shape==3~prob4),
           ...
           ch_shape=ch_shape+1,unch_shape=unch_shape+1,
           shape_left=shape_left+1,shape_right=shape_right+1,
           reoffer_ch=lag(ch_shape)==shape_right|lag(ch_shape)==shape_left,
           reoffer_unch=lag(unch_shape)==shape_right|lag(unch_shape)==shape_left,
           stay_key=ch_key==lag(ch_key),
           stay_shape=ch_shape==lag(ch_shape),
           stay_unch_shape=lag(unch_shape)==ch_shape,
